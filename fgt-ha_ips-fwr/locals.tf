locals {
  ...
  fgt-1_ni_private_ip  = cidrhost(var.subnet_cidrs["private1"], 10)

  fgt-2_ni_mgmt_ip    = cidrhost(var.subnet_cidrs["mgmt"], 11)
  fgt-2_ni_public_ip  = cidrhost(var.subnet_cidrs["public"], 11)
  fgt-2_ni_private_ip = cidrhost(var.subnet_cidrs["private1"], 11)
  # ----------------------------------------------------------------------------------
  # FAZ and FMG
  # ----------------------------------------------------------------------------------
  ...
  # iLB
  # ----------------------------------------------------------------------------------
  ilb_ip = cidrhost(var.subnet_cidrs["private1"], 9)
  # ----------------------------------------------------------------------------------
  # NCC
  # ----------------------------------------------------------------------------------
  ncc_private_ips = [
    cidrhost(var.subnet_cidrs["private1"], 5),
    cidrhost(var.subnet_cidrs["private1"], 6)
  ]
  ncc_public_ips = [
